...
class MainEngine(object):
    ...
    def connectAnother(self,fileName ):
        ...
        if gateway:
            gateway.connectAnother(fileName)
            
            # 接口连接后自动执行数据库连接的任务
            self.dbConnect()        
    #----------------------------------------------------------------------
    def connectJson(self,userID, password , brokerID,tdAddress,mdAddress):
        gatewayName = 'CTP'
        """连接特定名称的接口"""
        gateway = self.getGateway(gatewayName)
        
        if gateway:
            gateway.connectJson(userID, password , brokerID,tdAddress,mdAddress,authCode = None,userProductInfo = None)
